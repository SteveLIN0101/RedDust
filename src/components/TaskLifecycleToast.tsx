...
import type { TaskLocation, TaskOutcome } from "../data/types";
...
export type TaskLifecycleNotice = {
  ...
  status?: TaskOutcome["result"] | "queued" | "submitted";
  scoreLabel?: string;
...
const deltaPriority = ["trust", "safety", "signal", "blue_zone_evidence", "blueZoneEvidence", "stormReadiness", "water", "medicine", "outside_risk", "maintenance_debt", "failureDebt"];
...
export function TaskLifecycleToast({ notice }: TaskLifecycleToastProps) {
  ...
  const shownDeltas = visibleDeltas(notice.stateDelta ?? {});

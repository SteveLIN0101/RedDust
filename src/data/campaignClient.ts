...
export type CampaignDisplayPayload = {
  kind?: string;
  title?: string;
  text?: string;
  summary?: string;
  replay_text?: string;
  safe_observation_preview?: string;
  location?: string;
  day?: number;
  branch?: Branch;
  slot_id?: string;
...
export type CampaignDisplayEvent = {
  contract_version?: "reddust_campaign_display_v1" | string;
  display?: CampaignDisplayPayload;
  display_state?: Record<string, unknown>;
  metric_definitions?: Record<string, unknown>;
};

export type CampaignEvent = CampaignDisplayEvent & {
  seq: number;
  at: string;
  type:
    | "campaign_created"
    ...
    | "campaign_complete";
  campaign_id: string;
  payload: Record<string, unknown> & CampaignDisplayEvent;
};
...
export type CampaignReplayItem = CampaignDisplayEvent & {
  seq: number;
  phase_hint: string;
  ...
  replay_event?: Record<string, unknown> & CampaignDisplayEvent;
};

export type CampaignTrace = CampaignDisplayEvent & {
  campaign_id: string;
  status: string;
  ...
  ending?: Record<string, unknown> & CampaignDisplayEvent;
...
export function defaultCampaignApiBase() {
  ...
  return (params.get("api") || env?.VITE_REDDUST_API_BASE || "http://127.0.0.1:7001").replace(/\/$/, "");
}
